...
from src.state import Scenarios, ReportStateInput, ReportStateOutput, Sections, ReportState, SectionState, SectionOutputState, Queries
from src.prompts import scenario_generation_instructions, report_planner_query_writer_instructions, report_planner_instructions, query_writer_instructions, section_writer_instructions, final_section_writer_instructions
from src.configuration import Configuration
from src.utils import tavily_search_async, deduplicate_and_format_sources, format_sections
...
scenario_model = ChatOpenAI(model=Configuration.scenario_model, temperature=0) 


# Nodes
async def generate_scenarios(state: ReportState, config: RunnableConfig):
    """ Generate geopolitical risk & opportunity scenarios for the topic """
    topic = state["topic"]
    configurable = Configuration.from_runnable_config(config)
    scenario_structure = configurable.DEFAULT_SCENARIO_STRUCTURE

    system_instructions = scenario_generation_instructions.format(
        topic=topic,
        scenario_structure=scenario_structure
    )

    structured_llm = scenario_model.with_structured_output(Scenarios)
    results = structured_llm.invoke([SystemMessage(content=system_instructions)])

    return {"scenarios": results["scenarios"]}
